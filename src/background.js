...
function buildMessages(payload, provider) {
  let baseSystem =
    ...
    '- When [Full Page Content] is provided, do NOT rely only on the abstract or opening paragraphs. The document may have key information in the middle or end — scan the full content before answering.\n' +
    '- For every factual claim: if the claim is directly supported by text in [Key], [Context], or [Full Page Content], immediately append <src>verbatim span copied character-for-character from that source (≤40 words)</src> right after the claim. Never paraphrase inside <src>. If a factual claim has NO supporting text in the provided sources, append [unverified] after it. Only omit both markers for trivially obvious background facts (e.g. "water is wet").';
